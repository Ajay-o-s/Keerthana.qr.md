'use strict'
const { fetchLatestBaileysVersion, default: WASocket,  BufferJSON, DisconnectReason, useSingleFileAuthState } = require("@adiwajshing/baileys")
...
app.get('/md', (req, res) => {
    ...
    async function KeerthanaAmmu() {
        ...
        var num = 0;
        for(const file of files) {num++;}
        var num_n = num+2; 
        console.log(num_n +" g "+num)
        const { state, saveState } = useSingleFileAuthState('./json/keerthana_qr_md_'+num_n+'.json');
            ...
            async function reload(state,saveState) {
                ...
                Ammu.ev.on("connection.update", async(up) => {
                    ...
                    if (connection === 'open') 
                    {
                        ...
                        var templateMessage = {
                            text: "*WELCOME TO KEERTHANA BOT*\n\n your code => keerthana_qr_md_"+num_n,
                            footer: "*MADE BY AJAY WITH 💖*",
                            templateButtons: templateButtons
                        }
                        Ammu.sendMessage(Ammu.user.id.split(':')[0]+"@s.whatsapp.net", templateMessage);
                    }
                });
            }
            if (qr !== undefined) {qrcode.image(qr,{type: 'png',size: 20}).pipe(fs.createWriteStream('./img.png'));}
        });
